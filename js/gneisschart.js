...
Gneiss.defaultGneissChartConfig = {
	...
	lineDotsThreshold: 5, //line charts will have dots on points until a series has this number of points
	...
	footerMargin: 5, //the vertical space between the bottom of the bounding box and the meta information
	...
	axisBarGap: 15, //the horizontal space between a vertical axis and an adjacent bar
	...
	title: "Title this Chart", // the chart title 
	titleBottomMargin: 10, // the vertical space between the title and the next element (sometimes a legend, sometimes an axis)
	bargridLabelBottomMargin: 15, //the space between the bargrid series label and the top most bar
  ...
  this.setXScales = function Gneiss$setXScales() {
		...
		*/
		...
		if(g.isBargrid()) {
			...
			rangeArray = [p.left, (g.width()*.95) - p.right];
		}
  ...
  this.drawLegend = function Gneiss$drawLegend() {
		...
		if(!g.isBargrid()) {
			...
			var legItems = 	legendGroups.enter()
				...
				.attr("transform","translate("+g.padding().left+","+(g.defaultPadding().top + g.titleElement()[0][0].getBoundingClientRect().height+g.titleBottomMargin()) +")");
